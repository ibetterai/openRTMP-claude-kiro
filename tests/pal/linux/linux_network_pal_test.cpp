...
// Tests for Linux/Android Network PAL Implementation (epoll-based)
//
...
// =============================================================================
// Linux Network PAL Tests
// =============================================================================

class LinuxNetworkPALTest : public ::testing::Test {
protected:
    void SetUp() override {
        networkPal_ = std::make_unique<linux::LinuxNetworkPAL>();
    ...
    std::unique_ptr<linux::LinuxNetworkPAL> networkPal_;
};

TEST_F(LinuxNetworkPALTest, ImplementsINetworkPALInterface) {
...
TEST_F(LinuxNetworkPALTest, InitializeSucceeds) {
    linux::LinuxNetworkPAL freshPal;
    auto result = freshPal.initialize();
    EXPECT_TRUE(result.isSuccess());
}

TEST_F(LinuxNetworkPALTest, DoubleInitializeReturnsAlreadyInitialized) {
...
TEST_F(LinuxNetworkPALTest, CreateServerReturnsValidSocket) {
...
TEST_F(LinuxNetworkPALTest, CreateServerOnAllInterfaces) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, CreateServerWithReuseAddr) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, CreateServerOnInvalidAddress) {
    ServerOptions opts;
...
TEST_F(LinuxNetworkPALTest, SetSocketOptionNoDelay) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, SetSocketOptionKeepAlive) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, GetLocalAddressReturnsCorrectAddress) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, GetLocalPortReturnsCorrectPort) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, IsRunningInitiallyFalse) {
    EXPECT_FALSE(networkPal_->isRunning());
}

TEST_F(LinuxNetworkPALTest, StopEventLoopWhenNotRunning) {
    // Should not crash
    EXPECT_NO_THROW(networkPal_->stopEventLoop());
}

TEST_F(LinuxNetworkPALTest, RunEventLoopInThread) {
...
TEST_F(LinuxNetworkPALTest, AsyncAcceptRegistersCallback) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, AsyncReadRegistersCallback) {
...
TEST_F(LinuxNetworkPALTest, AsyncWriteRegistersCallback) {
...
TEST_F(LinuxNetworkPALTest, CloseSocketDoesNotCrash) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
TEST_F(LinuxNetworkPALTest, CloseInvalidSocketDoesNotCrash) {
...
TEST_F(LinuxNetworkPALTest, ClientServerCommunication) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
...
// =============================================================================
// epoll-Specific Tests
// =============================================================================

TEST_F(LinuxNetworkPALTest, HandleManyConnections) {
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
    opts.backlog = 256;
...
TEST_F(LinuxNetworkPALTest, EdgeTriggeredEventsWork) {
    // Test that edge-triggered mode works correctly
    uint16_t port = findAvailablePort();
    ServerOptions opts;
    opts.reuseAddr = true;
    ...
    // The implementation should handle edge-triggered mode correctly
    // This test primarily ensures the server can be created and used
