...
// Tests for Darwin (macOS/iOS) Thread PAL Implementation
//
...
// =============================================================================
// Darwin Thread PAL Tests
// =============================================================================

class DarwinThreadPALTest : public ::testing::Test {
protected:
    void SetUp() override {
        threadPal_ = std::make_unique<darwin::DarwinThreadPAL>();
    }
    ...
    std::unique_ptr<darwin::DarwinThreadPAL> threadPal_;
};

TEST_F(DarwinThreadPALTest, ImplementsIThreadPALInterface) {
...
TEST_F(DarwinThreadPALTest, CreateThreadReturnsValidHandle) {
...
TEST_F(DarwinThreadPALTest, ThreadExecutesFunction) {
...
TEST_F(DarwinThreadPALTest, CreateThreadWithName) {
    ThreadOptions opts;
...
TEST_F(DarwinThreadPALTest, JoinThreadWaitsForCompletion) {
...
TEST_F(DarwinThreadPALTest, DetachThreadAllowsIndependentExecution) {
    std::atomic<bool> completed{false};
...
TEST_F(DarwinThreadPALTest, CreateMultipleThreads) {
...
TEST_F(DarwinThreadPALTest, CreateThreadPoolReturnsValidHandle) {
...
TEST_F(DarwinThreadPALTest, SubmitWorkExecutes) {
    ThreadPoolOptions opts;
    opts.name = "WorkPool";
...
TEST_F(DarwinThreadPALTest, SubmitMultipleWorkItems) {
    ThreadPoolOptions opts;
...
TEST_F(DarwinThreadPALTest, DestroyPoolWaitsForActiveWork) {
    ThreadPoolOptions opts;
...
TEST_F(DarwinThreadPALTest, CreateMutexReturnsValidHandle) {
...
TEST_F(DarwinThreadPALTest, MutexLockUnlock) {
...
TEST_F(DarwinThreadPALTest, TryLockMutexWhenUnlocked) {
    auto mutexResult = threadPal_->createMutex();
    ASSERT_TRUE(mutexResult.isSuccess());
...
TEST_F(DarwinThreadPALTest, TryLockMutexWhenLocked) {
    auto mutexResult = threadPal_->createMutex();
    ASSERT_TRUE(mutexResult.isSuccess());
...
TEST_F(DarwinThreadPALTest, MutexProtectsSharedData) {
    auto mutexResult = threadPal_->createMutex();
    ASSERT_TRUE(mutexResult.isSuccess());
...
TEST_F(DarwinThreadPALTest, CreateConditionVariableReturnsValidHandle) {
...
TEST_F(DarwinThreadPALTest, ConditionVariableNotifyOneWakesOneThread) {
...
TEST_F(DarwinThreadPALTest, WaitConditionVariableForReturnsOnTimeout) {
    auto cvResult = threadPal_->createConditionVariable();
    auto mutexResult = threadPal_->createMutex();
    ASSERT_TRUE(cvResult.isSuccess());
    ASSERT_TRUE(mutexResult.isSuccess());
...
TEST_F(DarwinThreadPALTest, NotifyAllWakesAllThreads) {
    auto cvResult = threadPal_->createConditionVariable();
    auto mutexResult = threadPal_->createMutex();
    ASSERT_TRUE(cvResult.isSuccess());
    ASSERT_TRUE(mutexResult.isSuccess());
...
TEST_F(DarwinThreadPALTest, GetCurrentThreadIdReturnsNonZero) {
    ThreadId id = threadPal_->getCurrentThreadId();
    EXPECT_NE(id, ThreadId{0});
}

TEST_F(DarwinThreadPALTest, DifferentThreadsHaveDifferentIds) {
    ...
    auto result = threadPal_->createThread(
        [](void* arg) {
            ...
            // Convert pthread_t pointer to uint64_t using reinterpret_cast
            *id = reinterpret_cast<uint64_t>(pthread_self());
        },
        &otherThreadId,
        ThreadOptions{}
    );
...
TEST_F(DarwinThreadPALTest, SetThreadNameDoesNotCrash) {
    EXPECT_NO_THROW(threadPal_->setThreadName("TestThread"));
}

TEST_F(DarwinThreadPALTest, SleepForSleepsAtLeastSpecifiedDuration) {
    auto start = std::chrono::steady_clock::now();
